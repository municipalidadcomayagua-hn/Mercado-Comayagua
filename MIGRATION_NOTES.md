...
Aparte, el usuario corrió manualmente en Supabase un `delete` de `cobros` (tipo_cobro='mensual'), `cuentas_por_cobrar` y `abonos` para limpiar los meses ya guardados por locatarios registrados con el flujo viejo (confirmado que era solo una factura de prueba) — sin tocar `puestos` ni `deudas_mora`.

### Meses vigentes: de "Enero..mes actual por defecto" a "ninguno por defecto"
Tras esa limpieza, los meses seguían apareciendo en Pagos — no eran datos residuales, sino el comportamiento ya implementado en §23.4: `mesesVisibles` siempre incluía Enero..mes actual como tarjetas en blanco (sin guardar), independientemente de si existía algún cargo real. El usuario aclaró que el flujo debe ser 100% manual: **ningún mes se muestra por defecto**, solo los que ya tengan cargo guardado o los que el cobrador active a mano con "+ Agregar mes". Se quitó el bloque `mesesPorDefecto` (y el loop de precarga que dependía de él) en `loadPuestosGuardados` — `mesesVisibles` ahora es únicamente `Object.keys(pagosMensualesGuardados)`. La precarga de renta mensual (§23.3) sigue funcionando igual, solo que ahora se dispara exclusivamente desde `agregarMesVisible` (el botón "+ Agregar mes"), no también al cargar la pantalla.
