...
import { registrarAbono, type ResultadoRegistroAbono } from "@/lib/data/repositories/cuentas.repo";
...
import type { CobroConDetalle } from "@/lib/data/types";
...
/**
 * Modal para registrar un abono (pago parcial o de meses completos) contra
 * la cuenta de un locatario. Extraido de EstadoDeCuentaCobrador.tsx (mismo
 * comportamiento) para poder usarse tambien desde Pagos/PuestoCardMensual -
 * ver MIGRATION_NOTES.md. `onRegistrado` recibe el resultado (ya incluye el
 * recibo) - el llamador decide que hacer con el (mostrar ReciboAbono,
 * recargar datos, etc.), este componente solo se encarga de la captura.
 */
    ...
    try {
      const opciones =
        mesesSeleccionados.length > 0
          ? { meses: mesesSeleccionados, anio: anioActual, nombreCliente: nombreCliente ?? undefined }
          : {
      ...
      const resultado = await registrarAbono(mercadoId, numeroPuesto, monto, cobradorId, cobradorNombre, referenciaAbono || undefined, opciones);
      ...
      <ModalContent maxW={{ base: "100vw", md: "28rem" }} mx={{ base: 0, md: "auto" }}>
        ...
        <ModalBody>
          <VStack spacing={4} align="stretch">
            ...
            <Text fontWeight="bold">Saldo pendiente: {formatCurrency(saldoPendiente)}</Text>
